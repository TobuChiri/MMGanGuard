...
def create_densenet_model(model_type='densenet121', num_classes=2, pretrained=True):
    """
    创建DenseNet模型
    Args:
        model_type: DenseNet类型，可选 'densenet121', 'densenet169', 'densenet201', 'densenet161'
        num_classes: 类别数
    ...
    if model_type not in densenet_models:
        raise ValueError(f"不支持的DenseNet模型: {model_type}。支持的模型: {list(densenet_models.keys())}")

    # 创建模型
    model = densenet_models[model_type](pretrained=pretrained)
...
def compare_densenet_models():
    """比较不同DenseNet模型的性能"""

    parser = argparse.ArgumentParser(description='DenseNet模型性能对比')
    ...
    parser.add_argument('--save_dir', type=str, default='./checkpoints/densenet_comparison', help='模型保存目录')
    parser.add_argument('--num_workers', type=int, default=4, help='数据加载工作进程数')
    parser.add_argument('--device', type=str, default='cuda' if torch.cuda.is_available() else 'cpu', help='设备')
    parser.add_argument('--models', type=str, nargs='+',
                       default=['densenet121', 'densenet169', 'densenet201', 'densenet161'],
                       help='要比较的DenseNet模型列表')
    ...
    for model_type in args.models:
        ...
        if not args.force_retrain and check_training_completed(model_type, args.save_dir, args.num_epochs):
            ...
            try:
                ...
                checkpoint = torch.load(os.path.join(args.save_dir, model_type, 'latest_model.pth'), map_location='cpu', weights_only=False)
                ...
                result = {
                    ...
                    'total_params': sum(p.numel() for p in create_densenet_model(model_type=model_type, num_classes=2, pretrained=True).parameters())
                }
        ...
        # 创建模型
        model = create_densenet_model(model_type=model_type, num_classes=2, pretrained=True)
